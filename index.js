...
client.on("ready", () => {
  console.log(`Selam, ${client.user.username} olarak giriş yaptım!`);

  client.user.setActivity("✯ BY Gaming ❤️‍🔥 Bilal");
});
...
client.on("message", async message => {
  ...
  if (!message.guild) return;
        ...
        if (level) {
          ...
          message.channel
            .send(
              ` ${message.author.username}, Tebrikler ${user.level} seviye oldun!`
            )
...
let message = `${member}, ${member.guild.name}'e hoşgeldin!`
